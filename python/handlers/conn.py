...
def handle_conn(zeek_data):
    ...
    Responder assets are only created when proof of life exists (lazy creation).
    """
    ...
    # --- THE PORT VERIFICATION ENGINE (run first to determine proof of life) ---
    is_open = False
    ...
    # Always create the originator asset (it's a real device on the network)
    ...
    # --- Originator MAC processing (always safe, originator is real) ---
    if orig_ttl in [30, 64, 128, 255]:
        assign_mac(orig_asset, orig_mac)

    # Only create the responder asset if there is proof of life
    if is_open:
        ...
        # Log open port and run behavior inference
        ...
        # Track successful connections with port/protocol detail
        if resp_p is not None and proto in ('tcp', 'udp'):
        ...
        # No proof of life — only process originator MAC for router detection
        # (responder asset is NOT created, avoiding ghost IPs)
        if orig_ttl not in [30, 64, 128, 255]:
            same_subnet = (orig_ip.split('.')[:3] == resp_ip.split('.')[:3])
            if not same_subnet:
                retroactively_filter_router_mac(orig_mac)